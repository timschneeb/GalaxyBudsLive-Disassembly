...
public class XDMBuildCmd extends XDMHandleCmd implements XFOTAInterface {
        ...
        if (!((xDMWorkspace.authState == 1 || xDMWorkspace.authState == 0) && xDMWorkspace.serverAuthState == 1)) {
            if (!TextUtils.isEmpty(XDMAuth.xdmAuthCredType2String(xDMWorkspace.credType))) {
                if (!((xDMWorkspace.m_szUserName.length() == 0 && xDMWorkspace.m_szClientPW.length() == 0) || xDMWorkspace.credType == 2)) {
                    ...
                    xDMParserCred.m_szData = XDMAuth.xdmAuthMakeDigest(xDMWorkspace.credType, xDMWorkspace.m_szUserName, xDMWorkspace.m_szClientPW, xDMWorkspace.nextNonce, xDMWorkspace.nextNonce.length, (byte[]) null, 0);
    ...
    public static XDMParserAlert xdmAgentBuildCmdGenericAlert(XDMWorkspace xDMWorkspace, String str) {
        ...
        xDMParserAlert.itemlist = XDMList.xdmListAppend((XDMList) null, (XDMList) null, xDMParserItem);
        return xDMParserAlert;
    }
    ...
    public static XDMParserReplace xdmAgentBuildCmdReplace(XDMWorkspace xDMWorkspace, XDMLinkedList xDMLinkedList) {
        ...
        while (xDMParserItem != null) {
            ...
            if (xDMList == null) {
                xDMList = XDMList.xdmListAppend(xDMList, (XDMList) null, xDMParserItem2);
            } else {
                XDMList.xdmListAppend(xDMList, (XDMList) null, xDMParserItem2);
            }
    ...
    public static XDMParserStatus xdmAgentBuildCmdStatus(XDMWorkspace xDMWorkspace, int i, String str, String str2, String str3, String str4) {
        ...
        xDMParserStatus.m_szMsgRef = xDMWorkspace2.m_szMsgRef;
        xDMParserStatus.m_szCmdRef = String.valueOf(i);
        xDMParserStatus.m_szCmd = str;
        xDMParserStatus.m_szData = str5;
        if (!TextUtils.isEmpty(str3)) {
            xDMParserStatus.targetref = XDMList.xdmListAppend((XDMList) null, (XDMList) null, str3);
        } else if (xDMWorkspace2.targetRefList != null) {
            XDMLinkedList.xdmListSetCurrentObj(xDMWorkspace2.targetRefList, 0);
            XDMList xDMList = null;
            for (XDMParserItem xDMParserItem = (XDMParserItem) XDMLinkedList.xdmListGetNextObj(xDMWorkspace2.targetRefList); xDMParserItem != null; xDMParserItem = (XDMParserItem) XDMLinkedList.xdmListGetNextObj(xDMWorkspace2.targetRefList)) {
                XDMParserItem xDMParserItem2 = new XDMParserItem();
        ...
        if (!TextUtils.isEmpty(str2)) {
            xDMParserStatus.sourceref = XDMList.xdmListAppend((XDMList) null, (XDMList) null, str2);
        } else if (xDMWorkspace2.sourceRefList != null) {
            XDMLinkedList.xdmListSetCurrentObj(xDMWorkspace2.sourceRefList, 0);
            XDMList xDMList2 = null;
            for (XDMParserItem xDMParserItem3 = (XDMParserItem) XDMLinkedList.xdmListGetNextObj(xDMWorkspace2.sourceRefList); xDMParserItem3 != null; xDMParserItem3 = (XDMParserItem) XDMLinkedList.xdmListGetNextObj(xDMWorkspace2.sourceRefList)) {
                if (xDMList2 == null) {
                    xDMList2 = XDMList.xdmListAppend(xDMList2, (XDMList) null, xDMParserItem3);
                } else {
                    XDMList.xdmListAppend(xDMList2, (XDMList) null, xDMParserItem3);
                }
            }
            xDMParserStatus.sourceref = xDMList2;
        }
        if (("407".compareTo(str5) == 0 || "401".compareTo(str5) == 0) && i == 0) {
            XDMParserMeta xDMParserMeta = new XDMParserMeta();
            xDMParserMeta.m_szFormat = "b64";
            if (xDMWorkspace2.serverCredType == 0) {
                xDMParserMeta.m_szType = XDMInterface.CRED_TYPE_BASIC;
                Log.I("XDM_CRED_TYPE_BASIC");
            } else if (xDMWorkspace2.serverCredType == 1) {
                xDMParserMeta.m_szType = XDMInterface.CRED_TYPE_MD5;
                xDMParserMeta.m_szNextNonce = XDMBase64.xdmBase64Encode(xDMWorkspace2.serverNextNonce);
                Log.H("CRED_TYPE_MD5 serverNextNonce: " + new String(xDMWorkspace2.serverNextNonce, Charset.defaultCharset()) + "Encoded server nonce " + xDMParserMeta.m_szNextNonce);
            } else if (xDMWorkspace2.serverCredType == 2) {
                xDMParserMeta.m_szType = XDMInterface.CRED_TYPE_HMAC;
                xDMParserMeta.m_szNextNonce = XDMBase64.xdmBase64Encode(xDMWorkspace2.serverNextNonce);
            }
            xDMParserStatus.chal = xDMParserMeta;
        }
        if (!xDMWorkspace2.sendChal && XDMInterface.STATUS_AUTHENTICATIONACCEPTED.compareTo(str5) == 0 && i == 0 && xDMWorkspace2.serverCredType == 1) {
            ...
            xDMParserMeta2.m_szNextNonce = XDMBase64.xdmBase64Encode(xDMWorkspace2.serverNextNonce);
            XDBProfileAdp.xdbSetServerNonce(xDMParserMeta2.m_szNextNonce);
            Log.H("CRED_TYPE_MD5 serverNextNonce: " + new String(xDMWorkspace2.serverNextNonce, Charset.defaultCharset()) + "Encoded server nonce " + xDMParserMeta2.m_szNextNonce);
            xDMWorkspace2.sendChal = true;
            xDMParserStatus.chal = xDMParserMeta2;
        }
        if (xDMWorkspace2.serverCredType == 2 && "200".compareTo(str5) == 0 && i == 0) {
            ...
            xDMParserMeta3.m_szNextNonce = XDMBase64.xdmBase64Encode(xDMWorkspace2.serverNextNonce);
    ...
    public static XDMParserResults xdmAgentBuildCmdDetailResults(XDMWorkspace xDMWorkspace, int i, String str, String str2, String str3, int i2, char[] cArr) {
        ...
        xDMParserResults.itemlist = XDMList.xdmListAppend((XDMList) null, (XDMList) null, xDMParserItem);
