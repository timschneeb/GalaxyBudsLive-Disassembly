...
public class XDMResumeOperation {
        ...
        switch (xdbGetFUMOStatus) {
            ...
            case 250:
                ...
                if (XFOTADl.xfotaCopyGetDrawingPercentage()) {
                    XDMEvent.XDMSetEvent((Object) null, XUIEventInterface.DL_UIEVENT.XUI_DL_COPY_IN_PROGRESS);
                } else {
                    XDMEvent.XDMSetEvent((Object) null, XUIEventInterface.ACCESSORY_UIEVENT.XUI_DM_ACCESSORY_COPY_RETRY_CONFIRM);
                }
                return true;
            default:
                return false;
    ...
    private void startReportProcessIfNecessary(int i) {
        ...
        if (XDMAgent.xdmAgentGetSyncMode() != 0) {
            XDMToastHandler.xdmShowToast(String.format("%s%n%s", new Object[]{XDMDmUtils.getContext().getString(R.string.STR_DM_CONNECTING_SERVER), XDMDmUtils.getContext().getString(R.string.STR_COMMON_PLEASE_WAIT)}), 0);
            return;
        }
        if (i != 20) {
            if (i == 65 || i == 80 || i == 100) {
    ...
    private void startDLProcessIfNecessary(int i) {
        ...
        if (networkBlockType.isBlocked()) {
            ...
            if (XFOTADl.xfotaDownloadGetDrawingPercentage()) {
                XDMEvent.XDMSetEvent((Object) null, XUIEventInterface.DL_UIEVENT.XUI_DL_DOWNLOAD_IN_PROGRESS);
            ...
            if (shouldShowConfirm(ConfirmType.DOWNLOAD_CONFIRM)) {
                XDMEvent.XDMSetEvent((Object) null, XUIEventInterface.DL_UIEVENT.XUI_DL_DOWNLOAD_START_CONFIRM);
            } else {
