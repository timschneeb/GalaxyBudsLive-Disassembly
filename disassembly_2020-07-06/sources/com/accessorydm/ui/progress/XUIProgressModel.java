...
public class XUIProgressModel extends XUIBaseFullscreenModel {
    ...
    public String getProgressSizeText() {
        try {
            ...
            String format = String.format(Locale.getDefault(), "%.2f", new Object[]{Double.valueOf(((double) this.mProgressedDeltaSize) / 1048576.0d)});
            String format2 = String.format(Locale.getDefault(), "%.2f", new Object[]{Double.valueOf(((double) this.mTotalDeltaSize) / 1048576.0d)});
            return String.format(Locale.getDefault(), FotaProviderInitializer.getContext().getString(R.string.STR_ACCESSORY_PROGRESS_MB_PER_MB), new Object[]{format, string, format2, string});
        } catch (Exception e) {
            Log.E(e.toString());
            return "";
        }
