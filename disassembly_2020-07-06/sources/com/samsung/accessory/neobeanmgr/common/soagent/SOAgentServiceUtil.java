...
public class SOAgentServiceUtil {
    ...
    public static final String TAG = (Application.TAG_ + SOAgentServiceUtil.class.getSimpleName());
    /* access modifiers changed from: private */
    public static boolean mIsSelloutTaskRunning = false;
    /* access modifiers changed from: private */
    public static String testDeviceSN = "00000000000";
                    ...
                    public void run() {
                        if (!SOAgentServiceUtil.checkSNValidation(str) || !SOAgentServiceUtil.checkSNValidation(str2)) {
                            ...
                            if (str == null || str.equals("")) {
                                ...
                                new SOAgentServiceTask().execute(new String[]{str, str2, str3});
                            }
                        }
                    }
                });
    ...
    public static boolean checkSNValidation(String str) {
        ...
        for (int i = 0; i < charArray.length; i++) {
            if (charArray[i] <= 31) {
                String str2 = TAG;
                Log.d(str2, "SN has wrong character - inputCharArray[" + i + "]: " + charArray[i]);
                return false;
            }
        }
        return true;
    ...
    private static class SOAgentServiceTask extends AsyncTask<String, Void, Boolean> {
        ...
        public Boolean doInBackground(String... strArr) {
            ...
            boolean z = false;
            for (int i = 0; i < 3 && !(z = SOAgentService.getInstance().sendAccessoryInfo(FmmConstants.MODEL_NUMBER, this.leftSN, this.btAddress)); i++) {
            }
            if (!z) {
                return false;
            }
            for (int i2 = 0; i2 < 3; i2++) {
                z = SOAgentService.getInstance().sendAccessoryInfo(FmmConstants.MODEL_NUMBER, this.rightSN, this.btAddress);
                if (z) {
                    break;
                }
            }
        ...
        public void onPostExecute(Boolean bool) {
            String access$300 = SOAgentServiceUtil.TAG;
            Log.d(access$300, "SOAgentServiceTask End - result: " + bool);
