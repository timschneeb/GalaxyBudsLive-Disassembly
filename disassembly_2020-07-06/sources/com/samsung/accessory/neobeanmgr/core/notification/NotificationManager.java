...
public class NotificationManager {
    ...
    private static final String TAG = "NeoBean_NotificationManager";
    ...
    private class initAppListThread extends Thread {
        private Context mCt;
    ...
    public synchronized void initializeAppList(Context context) {
        ...
        int i = 0;
        while (i < NotificationConstants.mAlertApps.length) {
            try {
        ...
        for (ResolveInfo next : applicableApps) {
            ...
            if (!isPreDefinedApps(str) && !NotificationUtil.isExcludeApp(str) && !isExistApp(mNotificationAppList, str)) {
                if (!Util.isSamsungDevice()) {
                    if (str.contains("clock")) {
                        continue;
                    } else if (str.contains(NotificationCompat.CATEGORY_ALARM)) {
                    }
                }
                Log.d(TAG, "applicationInfo:pkgName() = " + str);
                try {
                    String charSequence = next.activityInfo.applicationInfo.loadLabel(context.getPackageManager()).toString();
        ...
        if (Util.isSamsungDevice() && Build.VERSION.SDK_INT >= 21) {
            for (Integer intValue : NotificationConstants.KNOX_USERID) {
                ...
                Log.d(TAG, "Get notificatios for user : " + intValue2);
                List<ResolveInfo> listOfApplicableApps = getListOfApplicableApps(context, intValue2);
                if (listOfApplicableApps != null) {
                    for (ResolveInfo next2 : listOfApplicableApps) {
                        String str3 = next2.activityInfo.applicationInfo.packageName;
                        String charSequence2 = next2.activityInfo.applicationInfo.loadLabel(context.getPackageManager()).toString();
                        ...
                        notificationAppData.setuId(intValue2);
    ...
    public static boolean isPreDefinedApps(String str) {
        for (String equalsIgnoreCase : NotificationConstants.mAlertApps) {
    ...
    private static List<ResolveInfo> getApplicableApps(Context context) {
        ...
        for (ResolveInfo add : context.getPackageManager().queryIntentActivities(intent2, 0)) {
            queryIntentActivities.add(add);
        }
    ...
    public void notifyListUpdated(Context context) {
        ...
        context.sendBroadcast(new Intent(NotificationConstants.ACTION_NOTIFICATION_LIST_UPDATE));
    }
    ...
    private static class AppListComparator implements Comparator<NotificationAppData> {
