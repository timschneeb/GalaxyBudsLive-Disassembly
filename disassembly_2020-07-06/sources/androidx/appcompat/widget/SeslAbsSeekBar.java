...
public abstract class SeslAbsSeekBar extends SeslProgressBar {
    ...
    public SeslAbsSeekBar(Context context, AttributeSet attributeSet, int i, int i2) {
        ...
        boolean z2 = obtainStyledAttributes.getBoolean(R.styleable.AppCompatSeekBar_useDisabledAlpha, true);
        ...
        this.mIsLightTheme = typedValue.data == 0 ? false : z;
    ...
    private void updateThumbAndTrackPos(int i, int i2) {
        ...
        if (drawable != null) {
            setThumbPos(i, drawable, getScale(), i5);
        }
        updateSplitProgress();
    }
    ...
    private void updateThumbAndTrackPosInVertical(int i, int i2) {
        ...
        if (drawable != null) {
            setThumbPosInVertical(i2, drawable, getScale(), i5);
        }
    }
        ...
        if (!ViewUtils.isLayoutRtl(this) || !this.mMirrorForRtl) {
            if (round >= getPaddingLeft()) {
                if (round <= width - getPaddingRight()) {
                    ...
                    setProgressInternal(Math.round(f + (f2 * ((float) max2))), true, false);
                }
                f2 = 1.0f;
                f = 0.0f;
                int max22 = getMax() - getMin();
                max = 1.0f / ((float) getMax());
                f3 = f2 % max;
                if (f3 > max / 2.0f) {
                }
                setHotspot((float) round, (float) round2);
                setProgressInternal(Math.round(f + (f2 * ((float) max22))), true, false);
            }
        } else if (round <= width - getPaddingRight()) {
            if (round >= getPaddingLeft()) {
                f2 = ((float) ((paddingLeft - round) + getPaddingLeft())) / ((float) paddingLeft);
                f = this.mTouchProgressOffset;
                int max222 = getMax() - getMin();
                max = 1.0f / ((float) getMax());
                f3 = f2 % max;
                if (f3 > max / 2.0f) {
                }
                setHotspot((float) round, (float) round2);
                setProgressInternal(Math.round(f + (f2 * ((float) max222))), true, false);
            }
            f2 = 1.0f;
            f = 0.0f;
            int max2222 = getMax() - getMin();
            max = 1.0f / ((float) getMax());
            f3 = f2 % max;
            if (f3 > max / 2.0f) {
            }
            setHotspot((float) round, (float) round2);
            setProgressInternal(Math.round(f + (f2 * ((float) max2222))), true, false);
        }
        f2 = 0.0f;
        f = 0.0f;
        int max22222 = getMax() - getMin();
        max = 1.0f / ((float) getMax());
        f3 = f2 % max;
        if (f3 > max / 2.0f) {
        }
        setHotspot((float) round, (float) round2);
        setProgressInternal(Math.round(f + (f2 * ((float) max22222))), true, false);
    }
